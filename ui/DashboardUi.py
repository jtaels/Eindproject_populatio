...
class DashboardUi:
    ...
    def _build_top_section(self):
        ...
        # Zorgt dat de kolommen evenredig schalen
    ...
    def _build_person_fiche_form(self, top_frame):
        ...
        ttk.Label(data_frame, text="Bevolkingsregisternr").grid(row=0, column=0, columnspan=2, padx=20, pady=(5, 0),
                                                                sticky="w")
        ...
        ttk.Label(data_frame, text="Geboorteplaats").grid(row=6, column=1, padx=20, pady=(5, 0), sticky="w")
        ttk.Entry(data_frame, width=20,textvariable=self._controller.person_fiche['geboorteplaats']).grid(row=5, column=1, padx=20, pady=5, sticky="ew")
        ...
        ttk.Label(data_frame, text="Overlijdensplaats").grid(row=7, column=1, padx=20, pady=(5, 0), sticky="w")
        ttk.Entry(data_frame, width=20,textvariable=self._controller.person_fiche['overlijdensplaats']).grid(row=7, column=1, padx=20, pady=5, sticky="ew")
        ...
        for action in self._controller.get_fiche_form_actions():
            ttk.Button(data_frame, text=action['name'], bootstyle=action['style'], command=action['command']).grid(row=fiche_btn_last_row,
                                                                                          columnspan=2, pady=5, padx=20,
                                                                                          sticky="ew")
    ...
    def _build_tree_view(self):
        ...
        ttk.Label(tree_frame, text="Treeview").grid(row=0, column=0, pady=5, sticky="w")
